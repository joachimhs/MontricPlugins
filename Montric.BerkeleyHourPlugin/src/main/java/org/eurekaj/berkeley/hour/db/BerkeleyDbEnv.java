...
*/
package org.eurekaj.berkeley.hour.db;
...
public class BerkeleyDbEnv extends EurekaJDBPluginService {
    ...
    public EntityStore getMetricHourStore() {
		return metricHourStore;
	}
    ...
    public void close() {
        ...
        if (metricHourStore != null) {
			try {
				metricHourStore.close();
			} catch (DatabaseException dbe) {
				log.error("Error closing metricHourStore" + dbe.toString());
                dbe.printStackTrace();
			}
		}
